...
# -----------------------------
BASE44_ENABLED = False  # Set to True after entering your valid API credentials
BASE44_API_KEY = "d16295d0c2c94d829f0bf2e661f276d5"
BASE44_READINGS_URL = "https://app.base44.com/apps/6a69685212e62119f92ab79f/editor/workspace/api"
BASE44_SUMMARIES_URL = "https://api.base44.com/v1/apps/YOUR_APP_ID/entities/ToolSummaries"

# Helper for non-blocking HTTP requests
def send_to_base44_async(url, payload):
    if not BASE44_ENABLED or BASE44_API_KEY == "YOUR_BASE44_API_KEY":
        return
    ...
    def _post():
        ...
        try:
            ...
            if resp.status_code not in (200, 201):
                print(f"[Base44 Error] {resp.status_code}: {resp.text}")
        except Exception as e:
            print(f"[Base44 Exception] {e}")

    # Launch in a background daemon thread so UI never freezes
...
last_base44_sync_time = 0.0  # Controls throttling for live telemetry sync
    ...
    # Case 2: Tool is currently ACTIVE
        ...
        # Current drops below Stop Threshold -> start tracking idle timeout
            ...
            if (time.time() - idle_start_time) >= IDLE_TIMEOUT:
                ...
                # --- SYNC SUMMARY TO BASE44 ---
                send_to_base44_async(BASE44_SUMMARIES_URL, {
                    "tool_id": summary["tool_id"],
                    "start_time": summary["start_clock"],
                    "duration_seconds": round(summary["duration"], 2),
                    "peak_current": round(summary["peak"], 2),
                    "min_life_pct": round(summary["min_life_pct"], 1),
...
# -----------------------------
def update():
    global filtered_current, last_base44_sync_time
    ...
    # Dynamically Update Threshold Lines on pyqtgraph Window
    baseline_line.setValue(active_cfg["sharp_baseline"])
    wear_line.setValue(active_cfg["wear_limit"])

    # --- SYNC LIVE TELEMETRY TO BASE44 (Throttled to once every 1 second) ---
    if BASE44_ENABLED and (now - last_base44_sync_time >= 1.0):
        last_base44_sync_time = now
        active_id = active_tool_data["tool_id"] if is_tool_active else "IDLE"
        send_to_base44_async(BASE44_READINGS_URL, {
            "timestamp": clock_str,
            "current_amp": round(filtered_current, 3),
            "tool_id": active_id,
            "status": tool_status
        })
