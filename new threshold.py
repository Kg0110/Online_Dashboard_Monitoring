...
# -----------------------------
TOOL_START_THRESHOLD = 2.5   # Minimum current (A) to START a new tool cycle
TOOL_STOP_THRESHOLD = 1.0     # Current (A) below which a tool is considered stopping/idle
...
def calculate_tool_life_metrics(current_val):
    ...
    """
    if current_val < TOOL_STOP_THRESHOLD:
...
# -----------------------------
# TOOL DETECTION ENGINE (WITH HYSTERESIS)
# -----------------------------
def process_tool_detection(current_t, clock_str, current_val):
    ...
    # Case 1: Start a new tool (Current >= 4.0A and inactive)
    if current_val >= TOOL_START_THRESHOLD and not is_tool_active:
        is_tool_active = True
        idle_start_time = None
        tool_counter += 1
    ...
    # Case 2: Tool is currently ACTIVE
        ...
        # Current stays above 2.5A -> reset idle timer (tool remains active)
        if current_val >= TOOL_STOP_THRESHOLD:
            idle_start_time = None

        # Current drops below 2.5A -> start tracking idle timeout
        else:
            if idle_start_time is None:
                idle_start_time = time.time()

            # Confirm tool STOP if below 2.5A for longer than IDLE_TIMEOUT
